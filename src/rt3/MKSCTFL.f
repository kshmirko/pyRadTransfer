...
CF2PY INTENT(OUT) TAUE_T, TAUS_T, SSA_T, EV_T
      ...
      REAL*8        X(NPTS), Y(NPTS), EV_A(0:NMOMS, 6)
      INTEGER       NMOMS, NPTS, IERR, I
      REAL*8        OMEGA_A, TAUS_A, YI(64)
      INCLUDE 'GAUSCOEF.f'
      EXTERNAL RAYEV, DISTR, WISCOMBE2EVANS, LEGVAL_A
     ...
     .     VOL, IERR)
      ...
      CALL LEGVAL_A(NMOMS, EV_A, 64, XI, YI)
      PRINT *, SUM(YI*XI*WEIGHT)/SUM(YI*WEIGHT)
...
C     однократного рассеяния
      ...
      CALL LEGVAL_A(NMOMS, EV_T, 64, XI, YI)
      PRINT *, SUM(YI*XI*WEIGHT)/SUM(YI*WEIGHT)
